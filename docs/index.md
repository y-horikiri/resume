...
### 2022/02 - : フィグニー株式会社
サーバーサイドエンジニアとして入社。  
2024年4月以降は、システムアーキテクチャ設計・技術選定・非機能要件の定義等を担当し、アーキテクト相当の役割を担う。
...
オンラインで処方箋を薬局に提出して、配送で医薬品を受け取れるサービスの新規開発。
サーバーサイドAPIとインフラをリードした。  
仕様の膨大さに対して期間がかなり短く、外部サービスを利用するなどで工数を削る工夫が求められた。（ビデオ通話機能やOCR機能など）
...
既存ECサイトにAPIとWebhookで連携して組み込む、商品をギフトとして贈れるサービスの新規開発。  
サーバーサイドAPIとバッチ、インフラをリードした。  
...
サーバーサイドAPIとバッチ、インフラ、旧システムからのデータ移行をリードした。  
...
AWS AppSyncを利用したGraphQL APIを中心とした、サーバーサイド、インフラの全てを基本設計から一人で担当した。  
...
レガシーな業務アプリの刷新。サーバーサイドAPI、バッチ、インフラを担当。  
...
チャットアプリの新規開発。サーバーサイドAPI、インフラを担当。
